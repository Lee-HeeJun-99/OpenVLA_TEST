#!/usr/bin/env python3
"""LOO surrogate tracing from projector.output delta to action-hidden correction."""
...
def make_plots(overall: list[dict[str, Any]]) -> None:
    ...
    plt.title("Projector→Hidden Surrogate Action Gap")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "projector_to_hidden_surrogate_gap.png", dpi=160)
    plt.close()
    ...
    plt.title("Projector→Hidden Surrogate Alignment")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "projector_to_hidden_surrogate_cosine.png", dpi=160)
    plt.close()
...
def make_report(overall: list[dict[str, Any]]) -> None:
    ...
    report = f"""# Projector→Hidden Surrogate Tracing
...
"""
    (OUTPUT_DIR / "projector_to_hidden_surrogate_report.md").write_text(report, encoding="utf-8")
...
[Outputs]
- `{OUTPUT_DIR / 'projector_to_hidden_surrogate_report.md'}`
- `{OUTPUT_DIR / 'projector_to_hidden_surrogate_frame_metrics.csv'}`
- `{OUTPUT_DIR / 'projector_to_hidden_surrogate_summary_overall.csv'}`
- `{OUTPUT_DIR / 'projector_to_hidden_surrogate_summary.json'}`
...
def main() -> int:
    ...
    write_csv(OUTPUT_DIR / "projector_to_hidden_surrogate_frame_metrics.csv", rows)
    write_csv(OUTPUT_DIR / "projector_to_hidden_surrogate_summary_overall.csv", overall)
    write_csv(OUTPUT_DIR / "projector_to_hidden_surrogate_summary_by_phase.csv", by_phase)
    make_plots(overall)
    make_report(overall)
    best = min(overall, key=lambda r: float(r["gap_to_sim_chunk_mean_l2_mean"]))
    summary = {
        "experiment": "Phase5 STEP 4 projector-to-hidden surrogate tracing",
        "policy_scope": "oftplus_h5_vision offline 225-pair dataset only",
        "frames": len(samples),
        "projector_feature": PROJECTOR_FEATURE,
        ...
        "outputs": {
            "frame_metrics": str(OUTPUT_DIR / "projector_to_hidden_surrogate_frame_metrics.csv"),
            "overall": str(OUTPUT_DIR / "projector_to_hidden_surrogate_summary_overall.csv"),
            "by_phase": str(OUTPUT_DIR / "projector_to_hidden_surrogate_summary_by_phase.csv"),
            "report": str(OUTPUT_DIR / "projector_to_hidden_surrogate_report.md"),
        },
        "interpretation_limits": [
            "Surrogate tensor-level tracing, not exact full-forward projector injection.",
            "Pooled projector deltas only.",
            "Offline action-head evidence only.",
        ],
    }
    write_json(OUTPUT_DIR / "projector_to_hidden_surrogate_summary.json", summary)
    append_log(summary)
