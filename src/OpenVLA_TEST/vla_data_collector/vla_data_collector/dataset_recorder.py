...
class DatasetRecorder(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("move_angular_velocity_deg_s", 20.0)
        self.declare_parameter("move_acceleration_mm_s2", 60.0)
        self.declare_parameter("move_angular_acceleration_deg_s2", 40.0)
        ...
        if (
            ...
            or self.move_angular_velocity <= 0
            or self.move_acceleration <= 0
            or self.move_angular_acceleration <= 0
        ):
    ...
    def _record_step(self) -> None:
        ...
        try:
            robot_tcp_pose = self.robot.get_tcp_pose().as_array()
            tcp_pose = self._convert_robot_pose_to_dataset_pose(
                robot_tcp_pose
            )
        except Exception as exc:
    ...
    def run_episode(self) -> None:
        ...
        self.get_logger().info("Setting robot mode: autonomous")
        self.robot.set_autonomous_mode()
        self.get_logger().info("Robot mode ready: autonomous")
        ...
        self.get_logger().info(f"Setting initial gripper state: {initial_gripper}")
        self.gripper.command(initial_gripper, wait=True)

        self._start_monotonic = time.monotonic()
        self._recording = True
        self.get_logger().info("Started episode recording")
        ...
        try:
            for waypoint in waypoints:
                ...
                self.get_logger().info(
                    ...
                    f"pose={target_pose}, "
                    f"vel=[{self.move_velocity}, "
                    f"{self.move_angular_velocity}], "
                    f"acc=[{self.move_acceleration}, "
                    f"{self.move_angular_acceleration}]"
                )
                self.robot.move_linear(
                    target_pose=target_pose,
                    velocity_mm_s=self.move_velocity,
                    acceleration_mm_s2=self.move_acceleration,
                    angular_velocity_deg_s=self.move_angular_velocity,
