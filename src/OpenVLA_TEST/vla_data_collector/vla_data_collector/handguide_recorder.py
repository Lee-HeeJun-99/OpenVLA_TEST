...
class HandguideRecorder(Node):
    """Record hand-guided demonstrations at a fixed sampler frequency."""
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("record_only_when_changed", True)
        ...
        self.episode_id = self._resolve_episode_id(
        ...
        self.record_only_when_changed = bool(
            self.get_parameter("record_only_when_changed").value
        )
        self.min_position_delta_m = float(
            self.get_parameter("min_position_delta_m").value
        )
        self.min_rotation_delta_rad = float(
            self.get_parameter("min_rotation_delta_rad").value
        )
        self.jpeg_quality = int(self.get_parameter("jpeg_quality").value)
        self.generate_actions_on_stop = bool(
            self.get_parameter("generate_actions_on_stop").value
        )
        ...
        self._latest_topic_pose_monotonic: Optional[float] = None

        self._gripper_lock = threading.Lock()
        self._recorded_gripper_state = int(
            self.get_parameter("initial_gripper_state").value
        )
    ...
    def _resolve_episode_id(self, configured_episode_id: str) -> str:
        ...
        return f"episode_{next_index:06d}"
    ...
    def _image_callback(self, msg: Image) -> None:
        with self._image_lock:
            ...
            self._latest_image_monotonic = time.monotonic()
    ...
    def _pose_callback(self, msg: Float64MultiArray) -> None:
        try:
            ...
            with self._pose_lock:
                self._latest_topic_pose = pose
                self._latest_topic_pose_monotonic = time.monotonic()
    ...
    def _read_robot_pose(self) -> np.ndarray:
        if self.pose_source == "service":
            return self.robot.get_tcp_pose().as_array()

        with self._pose_lock:
            if self._latest_topic_pose is None:
                raise RuntimeError(f"No pose received on {self.pose_topic}")
            return self._latest_topic_pose.copy()

    def _latest_image(self) -> tuple[np.ndarray, float, float]:
        with self._image_lock:
            if self._latest_image_msg is None or self._latest_image_monotonic is None:
                raise RuntimeError("No image available")
            msg = self._latest_image_msg
            received_monotonic = self._latest_image_monotonic
    ...
    def _image_msg_to_bgr_array(self, msg: Image) -> np.ndarray:
    ...
    def _should_record(self, tcp_pose: np.ndarray, gripper_state: int) -> bool:
        ...
        position_delta = float(np.linalg.norm(delta[:3]))
        rotation_delta = float(np.linalg.norm(delta[3:]))
        if position_delta >= self.min_position_delta_m:
            return True
        if rotation_delta >= self.min_rotation_delta_rad:
            return True
    ...
    def _record_step(self) -> None:
        ...
        try:
            robot_tcp_pose = self._read_robot_pose()
        ...
        if not self._should_record(tcp_pose, gripper_state):
            return
        ...
        record = {
            "step_index": index,
            "timestamp": now - self._start_monotonic,
        ...
        if self._step_index % 10 == 0:
            self.get_logger().info(
                f"recorded:{self._step_index} "
                f"gripper={gripper_state} image_age={image_age:.3f}s"
            )

    def _print_controls(self) -> None:
        print(
            "\n=== Hand-guided OpenVLA recorder ===\n"
            "Move the robot by hand. Keep motion continuous and avoid pauses.\n"
    ...
    def run_episode(self) -> None:
        ...
        self.get_logger().info(
            f"Started hand-guided recording: {self.episode_dir}, "
            f"sample_hz={self.frequency}, pose_source={self.pose_source}"
        )
        ...
        self.get_logger().info(
            f"Saved episode: {self.episode_dir} "
            f"({self._step_index} steps, success={success})"
        )
    ...
    def _write_metadata(self, success: bool) -> None:
        ...
        metadata = {
            ...
            "gripper_open_output_index": int(
                self.get_parameter("gripper_open_output_index").value
            ),
            "gripper_close_output_index": int(
                self.get_parameter("gripper_close_output_index").value
            ),
            "gripper_pulse_time_sec": float(
                self.get_parameter("gripper_pulse_time_sec").value
            ),
